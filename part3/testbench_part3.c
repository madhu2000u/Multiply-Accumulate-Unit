...
void filePrintHelper(FILE *inputData, FILE *expectedOutput, int a, int b, int f, bool valid_in, bool valid_out){
    ...
    fprintf(expectedOutput, "%07x, %x\n",f & 0xfffffff, valid_out);   //when the most significant hex value is 0, the c code does not print it out the %07x is the format specifier to print out the 0s as well.
...
int main()
{
    ...
    for(i = 0; i < saturationRangeTest; i++){
        ...
        input.b = 8191;         // Since the product, sum and other parts of the desin work correctly, this testing can indeed be considered comprehensive or throrough as we are only testing for saturation here.
        if(valid_in){
    ...
    saturationFluctuationCheckers(inputData, expectedOutput, 5800, 3800, input.f, valid_in, valid_out);


    for(i = 0; i < saturationRangeTest; i++){   // By keeping a resonably high saturationRangeTest value we can gaurantee that it will reach the -ve saturation from +ve saturation (provided the a and b are the max or fairly high values otherwise it will take more cycles or may not reach the other end of saturation).
        valid_out = false;
        valid_in = 1;
        input.a = 8191;
    ...
    // This manual testing is done to check whether the overflow value increases to a non-overflow value from the -ve saturation.
    ...
    saturationFluctuationCheckers(inputData, expectedOutput, 5800, -7777, input.f, valid_in, valid_out);
